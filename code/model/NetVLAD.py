...
Source : https://github.com/albanie/collaborative-experts/blob/master/model/net_vlad.py
"""
...
class NetVLADWrapper(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.net_vlad = NetVLAD(*args, **kwargs)

    def forward(self, x):
        # BGND -> (BG)ND
        if x.dim() == 4:
            G = x.shape[1]
            x = rearrange(x.contiguous(), 'b g n d -> (b g) n d')
            x = self.net_vlad(x)
            x = rearrange(x.contiguous(), '(b g) (i d) -> b g i d', g=G, i=1)
        elif x.dim() == 3:
            x = self.net_vlad(x)
            x = x.unsqueeze(1)
...
class NetVLADLinear(nn.Module):
    def __init__(self, cluster_size, feature_size, add_batch_norm=False):
        super().__init__()

        self.net_vlad = NetVLADWrapper(cluster_size, feature_size, add_batch_norm)
        self.out_linear = nn.Linear(cluster_size * feature_size, feature_size)

    def forward(self, x):
        x = self.net_vlad(x).contiguous().view(*x.shape[:-2], -1)
